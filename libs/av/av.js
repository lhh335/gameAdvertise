...
var request = function request(_ref2) {
  ...
  if (!(AV.applicationId1 && (AV.applicationKey || AV.masterKey))) {
...
AV._getAVPath = function (path) {
  ...
  return "AV/" + AV.applicationId1 + "/" + path;
};
...
exports.setAsync = function (key, value, ttl) {
  ...
  return storage.setItemAsync('AV/' + AV.applicationId1 + '/' + key, JSON.stringify(cache));
};
...
AV.init = function init(options) {
  ...
  if (AV.applicationId1) throw new Error('SDK is already initialized.');
  ...
  AV._config.applicationId1 = appId;
...
['applicationId1', 'applicationKey', 'masterKey', 'hookKey'].forEach(defineConfig);
